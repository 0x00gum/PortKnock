...
class PortKnocking(object):
    ...
    def _parse_args(self, args: list):
        ...
        parser.add_argument('-i', '--clientIp',
                            help='client Ip.', required=True)
        parser.add_argument('-n', '--numberOfknocks',
                            help='number of knocks to except form the client(default is 4): -n 4',
                            default=4,
                            required=False)
        parser.add_argument('-f', '--flag',
                            ...
                            required=True)
        parser.add_argument(
            '-c', '--cipher', help='cipher Key to decrypt the password (avoid MITM).', required=False)
    ...
    def get_encrypted_string(self) -> str:
        ...
        print("[+] Server is listening for incoming knocks.")
    ...
    def decrypt_icmp_packet(self):
        """Decrypting the ICMP packets sent by the client by using Fernet."""
    ...
    def decrypt_password(self) -> bool:
    ...
    def sniff_packets(self) -> None:
        ...
        if self.decrypt_password():
            ...
            sniffed_packets = sniff(
                count=self.numberOfknocks * 2, filter=sniff_filter)
            if sniffed_packets != " ":
